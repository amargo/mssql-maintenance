...
if ! [[ "${BACKUP_FULL_CLEANUP_TIME}" =~ ^[1-9][0-9]*$ ]]; then
    echo "$(date): BACKUP_FULL_CLEANUP_TIME must be a positive integer number of hours." >&2
    exit 1
fi
...
EXEC master.dbo.DatabaseBackup
  ...
  @CleanupTime = ${BACKUP_FULL_CLEANUP_TIME},
  @LogToTable  = 'Y';"
